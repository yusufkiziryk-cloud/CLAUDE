      ...
      {isPin ? (
        <>
          ...
          <div className={clsx('flex gap-4 mb-10 transition-transform', shake && 'animate-bounce')}>
            {dots.map(i => (
              <div key={i} className={clsx('w-4 h-4 rounded-full border-2 transition-all duration-150', i < input.length ? (error ? 'bg-red-500 border-red-500' : 'bg-indigo-400 border-indigo-400') : 'border-slate-600')} />
            ))}
          </div>
