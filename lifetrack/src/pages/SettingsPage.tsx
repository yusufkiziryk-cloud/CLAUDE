...
import { Download, Upload, Trash2, Plus, Database, Folder, Lock, Unlock, Brain, Eye, EyeOff, HardDrive, CheckCircle2, AlertCircle, RefreshCw } from 'lucide-react'
import toast from 'react-hot-toast'
import { useStore } from '../lib/store'
import { exportJSON, importJSON } from '../utils/export'
import { useFileSync } from '../hooks/useFileSync'
import { isSupported } from '../lib/file-storage'
...
export default function SettingsPage() {
  ...
  const { status: syncStatus, fileName: syncFileName, lastSaved: syncLastSaved, connect: syncConnect, setupNew, setupExisting, disconnect: syncDisconnect, saveNow } = useFileSync()
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      {/* Local File Storage */}
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5">
        <h2 className="font-semibold mb-1 flex items-center gap-2"><HardDrive size={16} /> Yerel Veri Dosyası</h2>
        <p className="text-xs text-slate-400 mb-4">Verilerinizi bilgisayarınızda gerçek bir .json dosyasında saklayın. Her değişiklik otomatik kaydedilir.</p>

        {!isSupported() ? (
          <div className="flex items-center gap-2 p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-xl text-sm text-yellow-700 dark:text-yellow-400">
            <AlertCircle size={15} />
            <span>Bu tarayıcı Dosya Sistemi API'sini desteklemiyor. Chrome veya Edge kullanın.</span>
          </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2 p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium text-green-700 dark:text-green-300 truncate">{syncFileName}</div>
                {syncLastSaved && <div className="text-xs text-green-600 dark:text-green-500">Son kayıt: {syncLastSaved.toLocaleTimeString('tr-TR')}</div>}
              </div>
            </div>
            <div className="flex gap-2">
              <button onClick={saveNow} className="flex-1 flex items-center justify-center gap-1.5 py-2 text-sm bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 rounded-lg transition-colors">
                <RefreshCw size={13} /> Şimdi Kaydet
              </button>
              <button onClick={() => { syncDisconnect(); toast.success('Dosya bağlantısı kesildi') }} className="flex-1 flex items-center justify-center gap-1.5 py-2 text-sm bg-red-50 dark:bg-red-950/30 hover:bg-red-100 dark:hover:bg-red-950/50 border border-red-200 dark:border-red-800 text-red-600 dark:text-red-400 rounded-lg transition-colors">
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2 p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-xl text-sm">
              <AlertCircle size={15} className="text-yellow-600 dark:text-yellow-400 shrink-0" />
              <div>
                <div className="font-medium text-yellow-700 dark:text-yellow-300">{syncFileName}</div>
                <div className="text-xs text-yellow-600 dark:text-yellow-500">Sayfa yenilendi — dosyaya erişmek için izin gerekiyor.</div>
              </div>
            </div>
            <button onClick={async () => { await syncConnect(); toast.success('Dosyaya bağlandı') }} className="w-full flex items-center justify-center gap-2 py-2.5 bg-primary-600 hover:bg-primary-700 text-white text-sm rounded-lg transition-colors">
              <HardDrive size={14} /> Dosyaya Bağlan
            </button>
          </div>
        ) : (
          <div className="space-y-2">
            <button onClick={async () => { await setupNew(); toast.success('Yeni dosya oluşturuldu ve bağlandı') }} className="w-full flex items-center gap-3 px-4 py-3 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-xl transition-colors text-left">
              <div className="w-9 h-9 bg-primary-100 dark:bg-primary-900/40 rounded-lg flex items-center justify-center shrink-0"><HardDrive size={16} className="text-primary-600" /></div>
              <div>
                <div className="font-medium text-sm">Yeni Dosya Oluştur</div>
                <div className="text-xs text-slate-500">Mevcut verilerinizi yeni bir .json dosyasına aktar</div>
              </div>
            </button>
            <button onClick={async () => { await setupExisting(); toast.success('Dosya bağlandı ve veriler yüklendi') }} className="w-full flex items-center gap-3 px-4 py-3 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-xl transition-colors text-left">
              <div className="w-9 h-9 bg-blue-100 dark:bg-blue-900/40 rounded-lg flex items-center justify-center shrink-0"><Folder size={16} className="text-blue-600" /></div>
              <div>
                <div className="font-medium text-sm">Mevcut Dosyayı Aç</div>
                <div className="text-xs text-slate-500">Daha önce oluşturulmuş LifeTrack dosyasını bağla</div>
              </div>
            </button>
          </div>
        )}
